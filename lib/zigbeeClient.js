...
class ZigbeeClient extends EventEmitter {
    constructor() {
        super();
    ...
    getFirmware(addr, endpoint, callback) {
        let data = `FE0A010102${addr}${endpoint}88`;
        this.serialClient.send(data, SIGNALTYPE.FIRMWARE, callback);
    }
    ...
    restartCoordinator(addr, endpoint) {
        let data = `FE0B0103020000008803`;
        this.serialClient.send(data, null);
    }
