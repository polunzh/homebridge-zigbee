...
function openNetwork(addr) {
    var command = 'fe0b020102' + addr + '0088ff';
...
function endpointAddressHandler(data) {
    ...
    var k = _.findKey(deviceAddress, { addr: addr });
...
function getEndPoint(addr) {
    ...
    port.write(generateCommand(data), function (err) {
        if (err) throw err;
...
function openLight(macAddress, callback) {
    ...
    var data = 'fe0b050102' + addr + port + '8801';
